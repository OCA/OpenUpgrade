...
//		}
		...
		if ($tiny_product['haspic']==1) {
			...
			mysql_query("update products set products_image='".$tiny_product['fname']."' where
				products_id=".$oscom_id.";");
		}
		else
		{
			mysql_query("update products set products_image=NULL where
				products_id=".$oscom_id.";");
		}
...
//		}
	...
	function get_partner_address($address_condition)
	{
		$addresses = array();
	...
	function get_saleorders($last_so) {
		...
		if ($result){
			while ($row=mysql_fetch_row($result)) {
				...
				$cust_names = split(" ",$row[1]);
				$default_condition = array("customers_id"=>$row[25],
							"entry_company"=>$row[26],
							"entry_firstname"=>$cust_names[0],
							"entry_lastname"=>$cust_names[1],
							"entry_street_address"=>$row[2],
							"entry_city"=>$row[3],
							"entry_postcode"=>$row[4]
							);
				$default_address = get_partner_address($default_condition);
				$delivery_names = split(" ",$row[9]);
				$delivery_condition = array("customers_id"=>$row[25],
							"entry_company"=>$row[27],
							"entry_firstname"=>$delivery_names[0],
							"entry_lastname"=>$delivery_names[1],
							"entry_street_address"=>$row[10],
							"entry_city"=>$row[11],
							"entry_postcode"=>$row[12]
							);
				$delivery_address = get_partner_address($delivery_condition);
				$billing_names = split(" ",$row[15]);
				$billing_condition = array("customers_id"=>$row[25],
							"entry_company"=>$row[28],
							"entry_firstname"=>$billing_names[0],
							"entry_lastname"=>$billing_names[1],
							"entry_street_address"=>$row[16],
							"entry_city"=>$row[17],
							"entry_postcode"=>$row[18]
							);
				$billing_address = get_partner_address($billing_condition);
				...
				$saleorders[] = new xmlrpcval( array("id" =>		new xmlrpcval( $row[0], "int"),
													...
													"pay_met" =>		new xmlrpcval( $row[24], "string"),
