...
def _country_info(self, cr, uid, data, context):
    country_obj = pooler.get_pool(cr.dbname).get('res.country')
...
def _state_info(self, cr, uid, data, country_id, context):
    state_obj = pooler.get_pool(cr.dbname).get('res.country.state')
...
def _do_import_website(self, cr, uid, website_id, context):
        ...
        for orderline in saleorder['lines']:
            ...
            if len(ids):
                ...
                if orderline['tax_rate'] > 0.0000:
                    tax_rate_search_ids = tax_obj.search(cr, uid, [('tax_group','=','vat'),('amount','=',orderline['tax_rate']/100)])
...
def _do_import(self, cr, uid, data, context):
    ...
    if data['model'] == 'ir.ui.menu':
        ...
        for website_id in website_ids:
            no_of_so += _do_import_website(self, cr, uid, website_id, context)
    else:
        no_of_so += _do_import_website(self, cr, uid, data['id'], context)
