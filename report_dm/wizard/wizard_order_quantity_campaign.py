...
_order_quantity_campaign_form = """<?xml version="1.0"?>
...
class wizard_qty_campaign_report(wizard.interface):
    states = {
        'init': {
            'actions': [_get_value],
            'result': {'type':'form', 'arch':_order_quantity_campaign_form, 'fields':_order_quantity_campaign_fields, 'state':[('end','Cancel'),('print','Print Report')]},
        },
        'print': {
            'actions': [],
            'result': {'type':'print', 'report':'dm.order.quantity.campaign', 'state':'end'},
        },
    }
wizard_qty_campaign_report('dm.order.quantity.campaign')
