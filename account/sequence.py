...
class ir_sequence(osv.osv):
	...
	if not context:
		context = {}
	test2 = test + """ AND (EXISTS (SELECT 1 FROM account_sequence_fiscalyear 
			WHERE sequence_main_id = ir_sequence.id AND fiscalyear_id = %d ) 
		OR NOT EXISTS (SELECT 1 FROM account_sequence_fiscalyear 
			WHERE sequence_main_id = ir_sequence.id) ) """ % context.get('fiscalyear_id', False)
        return super(ir_sequence, self).get_id(cr, uid, sequence_id, test2, context)
ir_sequence()
