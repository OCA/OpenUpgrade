...
class third_party_ledger(report_sxw.rml_parse):
    ...
    def lines(self, partner):
        ...
        self.cr.execute(
                "SELECT l.date, j.code, l.ref, l.name, a.name as aname, l.debit, l.credit " \
                "FROM account_account a, account_move_line l " \
                "LEFT JOIN account_journal j " \
                    "ON (l.journal_id = j.id) " \
                "WHERE l.partner_id = %d AND l.account_id = a.id " \
...
report_sxw.report_sxw('report.account.third_party_wise_ledger', 'res.partner',
        'addons/account_india_ledger/report/third_party_ledger.rml',parser=third_party_ledger,
        header=False)