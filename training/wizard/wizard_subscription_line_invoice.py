...
#        return inv_id
#
#    for line in pool.get('training.subscription.line').browse(cr,uid,data['ids']):
#        if (not line.invoiced) and (line.subscription_id.state not in ('draft','cancel')):
#            if not line.subscription_id.id in invoices:
#                invoices[line.subscription_id.id] = []
