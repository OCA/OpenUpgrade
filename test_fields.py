...
def get(object, level=3, ending=[], ending_excl=[], recur=[], root=''):
	res = sock.execute('terp', 3, 'admin', 'res.partner', 'fields_get')
	key = res.keys()
	key.sort()
	for k in key:
...
get('account.invoice', level=1, ending_excl=['one2many','many2one','many2many','reference'], recur=['many2one'])

print
print 'Field selection for a repeatIn', '='*40
get('account.invoice', level=1, ending=['one2many','many2many'], recur=['many2one'])
