...
class report_commission_month(osv.osv):
    ...
    def init(self, cr):
        ...
        cr.execute("""
            create or replace view report_commission_month as (
              ...
              as commission,I.state,AM.date_created as pdate from account_invoice_line L,account_move_line AM,
              sale_agent A,res_partner P,account_invoice I,product_pricelist_item R,sale_order_line SL,product_pricelist_version PV,
              sale_order S, product_pricelist PP where S.pricelist_id=PP.id AND  R.price_version_id=PV.id AND PP.id = PV.pricelist_id AND I.origin=S.name AND SL.order_id=s.id AND R.price_discount > 0 AND
               S.partner_id = P.id AND I.partner_id=P.id AND P.agent_id=A.id AND I.id=L.invoice_id  group by R.price_discount,I.state,I.id,A.name,P.name,I.state,I.date_invoice,
