...
class payroll_setup_deductions(osv.osv):
    _name = "payroll.setup.deductions"
    _description = "Deductions"
    _columns = {
            'name' : fields.char('Element ID', size=32, required=True),
            'element_type' : fields.many2one("payroll.setup.elements", 'Element Type', domain=[('category','=','deductions')]),
