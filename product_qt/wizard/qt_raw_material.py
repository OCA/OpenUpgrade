...
def _get_cases(self, cr, uid, data, context):
    move_obj=pooler.get_pool(cr.dbname).get('stock.move').browse(cr,uid,data['id'])
    prod_obj=pooler.get_pool(cr.dbname).get('product.product').browse(cr,uid,move_obj.product_id.id)
    fields.clear()
