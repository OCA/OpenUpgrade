...
class analytic_journal_rate_grid(osv.osv):
	...
	_columns={
		...
		'rate_id': fields.many2one("hr_timesheet_invoice.factor", "Invoicing Rate",required=True,),
		}
...
class account_analytic_account(osv.osv):
	...
	_columns = {
		'journal_rate_ids' : fields.one2many('analytic_journal_rate_grid', 'account_id', 'Journal/Invoicing Rate Rel.'),
	}
...
class hr_analytic_timesheet(osv.osv):
	...
	def on_change_account_id(self, cr, uid, ids,user_id, account_id, journal_id):
		res = {}
		if not (account_id):
		...
		if not temp:
			...
			res['value']= super(hr_analytic_timesheet, self).on_change_account_id(cr, uid, ids,user_id, account_id)['value']
...
class account_invoice(osv.osv):
	...
	def _get_analityc_lines(self, cr, uid, id):
		...
		if inv.type == 'in_invoice' or True:
			for il in iml:
