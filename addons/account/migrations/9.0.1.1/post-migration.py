...
def fill_move_taxes(env):
    ...
    If a tax code is used as a tax code, select the first tax. Highest\
    sequence and active taxes preferred. Set this tax as the line's
    tax_line_id (Originator tax).

    else, if a tax code is used as a base code, select the first tax using
