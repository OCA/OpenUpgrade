...
def create_workitem_picking(cr, uid, pool):
    ...
    # Activity
    cr.execute("""SELECT id FROM wkf_activity WHERE name = %s AND action = %s""",
               ('picking', 'action_picking_create()',))
    res = cr.fetchone()
    wkf_activity = res and res[0]

    # Get the not shipped moves linked to any Purchase Order
    cr.execute("""SELECT purchase_line_id FROM stock_move WHERE purchase_line_id is not null AND state != %s""",
               ('done',))
    for res in cr.fetchall():
        ...
        # Create workItem
        cr.execute("""SELECT id FROM wkf_workitem WHERE act_id = %s AND inst_id = %s""" % (wkf_activity, wkf_instance))
        res2 = cr.fetchone()
        wkf_workitem = res2 and res2[0]
        if not wkf_workitem:
            cr.execute("""INSERT INTO wkf_workitem (act_id, inst_id, state) VALUES (%s, %s, %s)""",
...
def migrate_product_supply_method(cr):
    ...
    if mto_route_id:
        product_ids = []
        cr.execute("SELECT id FROM product_template WHERE %s = %%s" % procure_method_legacy, ('buy',))
        for res in cr.fetchall():
...
def migrate_procurement_order(cr):
    '''
    Switch the old purchase_id in Procurement Order for the new field Purchase Order Line
    :param cr: Database cursor
    '''
    pool = pooler.get_pool(cr.dbname)
    ...
    for po_id, purchase_id in res:
        ...
        cr.execute("""SELECT id FROM purchase_order_line WHERE order_id = %s AND product_qty = %s AND
                    product_id = %s""", (purchase_id, procurement.product_qty, procurement.product_id.id))
        res2 = cr.fetchone()
