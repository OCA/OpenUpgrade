...
def _fast_fill_account_move_line_tax_tag_invert(env):
    ...
    openupgrade.logged_query(
        ...
        SET tax_tag_invert =
            CASE
                WHEN (SELECT COUNT(tag_rel.account_account_tag_id)
                    FROM account_account_tag_account_move_line_rel tag_rel
                    WHERE aml.id = tag_rel.account_move_line_id) > 0
        ...
        WHERE aml.tax_repartition_line_id IS NULL
            AND (
                SELECT COUNT(tax_rel.account_tax_id)
                FROM account_move_line_account_tax_rel tax_rel
                WHERE aml.id = tax_rel.account_move_line_id) = 0
