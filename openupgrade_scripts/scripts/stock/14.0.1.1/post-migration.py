...
def run_lovefurniture(env):
	for picking in env['stock.picking'].search([('picking_type_id.code','=','outgoing')]):
		if picking.move_lines and picking.move_lines[0].location_id != picking.location_id:
			picking.write({'location_id' : picking.move_lines[0].location_id.id})
