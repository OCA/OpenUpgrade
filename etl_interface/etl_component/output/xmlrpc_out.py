...
class etl_component_xmlrpc_out(osv.osv):
    _name = 'etl.component'
    ...
    def create_instance(self, cr, uid, id, context={}, data={}):
        val = super(etl_component_xmlrpc_out, self).create_instance(cr, uid, id, context, data)
        obj_connector = self.pool.get('etl.connector')
        obj_transformer = self.pool.get('etl.transformer')
        cmp=self.browse(cr, uid, id)
        if cmp.type_id.name == 'output.xmlrpc_out':
            ...
            val = etl.component.input.xmlrpc_out(conn_instance, cmp.name, trans_instance, cmp.row_limit)
        return val

etl_component_xmlrpc_out()
