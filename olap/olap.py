...
class olap_fact_database(osv.osv):
    ...
    _columns = {
        ...
        'db_password': fields.char('Database password', size=64, invisible=True, required=True),
...
#                result.append( (t.id, t.name+' ('+t.column_db_name+')'))
...
#       if context and context.has_key('link_col') and context['link_col']:
#           if args:
#               ids = args[0][2][0][2]
#           if not ids:
    ...
    def _available_table_get(self, cr, uid, ids, name, arg, context=None):
        ...
        for table in self.browse(cr, uid, ids, context):
            if table.line_ids:
                ...
                result[table.id]=[]
#           else:
#               if context and context.has_key('d_id') and context['d_id']:
#                   dim_obj = self.pool.get('olap.dimension').browse(cr,uid,int(context['d_id']))
#                   context['d_id']=False
#                   ids = []
#                   for line in dim_obj.cube_id.table_id.line_ids:
#                       ids.append(line.table_id.id)
#                   result[table.id] = field_obj.search(cr, uid, [('table_id','in', ids),('hide','<>',True),('active','<>',False)], context=context)
#               elif context and context.has_key('parent_schema_id') and context['parent_schema_id']:
#                   schema_id = self.pool.get('olap.schema').browse(cr,uid,context['parent_schema_id'])
#                   table_ids = self.pool.get('olap.database.tables').search(cr,uid,[('fact_database_id','=',schema_id.database_id.id)])
#                   result[table.id] = field_obj.search(cr, uid, [('table_id','in',table_ids),('primary_key','=',True),('hide','<>',True),('active','<>',False)], context=context)
#               else:
#                   result[table.id] = field_obj.search(cr, uid, [('hide','<>',True),('active','<>',False)], context=context)
        return result
...
#        'name':_set_name,
#        'available_table_ids': lambda self, cr, uid, context: self.pool.get('olap.database.columns').search(cr, uid, [('primary_key','=',True),('hide','<>',True)], context=context)
#        'available_table_ids':_def_set,
    }
    def field_add(self, cr, uid, ids, context={}):
        return {}
olap_cube_table()
...
class olap_cube(osv.osv):
    ...
    _columns = {
        ...
        'table_id': fields.many2one('olap.cube.table', 'Fact table',size=64,required=True),
        ...
        'query_log': fields.boolean('Query Logging'),
...
#        'field_name': fields.char('Hierarchy Field name',size=64,required=True),   # To remove ?
#        'member_all': fields.char('All member',size=64,required=True),
#        'member_default': fields.char('Default member',size=64, required=True),
        'sequence': fields.integer('Sequence',required=True),
#        'type': fields.selection(_hierarchy_get, 'Hierarchy name',size=64,required=True),
        ...
        'table_id': fields.many2one('olap.cube.table', 'Fact table(s)', required=True),
    }
    _defaults = {
        'sequence': lambda *args: 1,
#        'member_all': lambda *args: 'all',
#        'member_default': lambda *args: 'all',
        'primary_key': lambda *args: 'id',
#        'type':lambda *args: 'many2one',
...
#    'column_id_name':fields.many2one('olap.database.columns','Column ID', required=True),
