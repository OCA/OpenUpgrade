...
#								if type[i] in ('float','FLOAT','Float'):
# 									temp = "'%.2f'%"
# 									temp1 = "%s['%s%d'] ]]"%(name,v,t)
# 									if v=="sum_debit" or v=="sum_credit":
# 										newnode.childNodes[1].lastChild.data = "[[" + temp +temp1
# 									else:
# 									 	newnode.childNodes[1].lastChild.data = "[[" + temp +temp1 + "[[" + temp2
...
#			self.result_total = {}
	...
	def linesForTotal(self,form,ids={},doneAccount={},level=1):
		...
		for id in form['fiscalyear'][0][2]:
			...
			tmp[0].debit,tmp[0].credit,tmp[0].balance
			if len(tmp):
				accounts.append(tmp)
	...
	def lines(self, form, ids={}, done={}, level=1):
		...
		for id in form['fiscalyear'][0][2]:
			...
			tmp1[0].debit,tmp1[0].credit,tmp1[0].balance
...
#					if not (res['credit'] or res['debit']) and not account[j].child_id:
#						continue
