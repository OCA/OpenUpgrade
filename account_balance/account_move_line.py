		...
		if context.get('periods', False):
			...
			if not 'period_manner' in context:
				...
				if context['period_manner']=='actual':
					...
					strQuery = obj+".active AND "+obj+".state<>'draft' AND((" +obj+".period_id " + p_id

					periods = self.pool.get('account.period').read(cr,uid,[ids],['date_start','date_stop'])

					for period in periods:
						strQuery += " or ("+obj+".create_date between to_date('" + period['date_start']  + "','yyyy-mm-dd') and to_date('" + period['date_stop']  + "','yyyy-mm-dd'))"
